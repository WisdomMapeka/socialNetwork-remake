...
function ChatSection(){
    ...
    useEffect(
        ...
        let deleteMessage = (mesage) =>{
            let mesage_data = {
                data:{id:mesage.id}
            }
            axios.delete(`${BASE_URL_DEV}/savemessagepic/`, mesage_data, header_values)
            .then((res) => {
            console.log(res)
            window.location.reload()
            })
        ...
        return (
            <>
                <div className="flex-1 max-w-2xl px-6 py-4 justify-between flex flex-col h-screen bg-white">
                    ...
                    <div className="relative  flex items-center space-x-4">
                        ...
                        <div className="flex flex-col leading-tight">
                            <div className="text-sm mt-1 flex items-center">
                                <span className="text-gray-900 text-xl mr-3">{chatParticipants.chatReceiver.first_name + " " + chatParticipants.chatReceiver.last_name}</span>
                            </div>
                            <span className="text-xs text-gray-600">
                                {chatParticipants.chatReceiver.location}
                            </span>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="chat-message" key={mesage.id}>
                            {mesage.sender === chatParticipants.chatStarter.user ? 
                                <div className="flex items-end">
                                    {mesage.is_pic === true ? 
                                        <div className="max-w-xl flex flex-col space-y-2 text-xs mx-2 order-2 items-end">
                                            <img src={mesage.pic} alt="" />
                                            <div className="text-xs text-gray-500">{cleanDate(mesage.date_created)}</div>
                                        </div>
                                        
                                    : 
                                    <div className="flex flex-col space-y-2 text-xs max-w-xs mx-2 order-2 items-end">
                                        <div>
                                            <span className="px-4 py-2 rounded-lg inline-block rounded-bl-none bg-gray-300 text-gray-600">{mesage.message}</span>
                                        </div>
                                        <div className="text-xs text-gray-500">{cleanDate(mesage.date_created)}</div>
                                    </div>
                                    }
                                    <div className="bg-white p-1 text-xs text-gray-500">
                                        <span className="cursor-pointer" alt="delete" onClick={() => deleteMessage(mesage)}><RiDeleteBinLine /></span>
                                    </div>
                            ...
                            : 
                                <div className="flex items-end justify-end">
                                    {mesage.is_pic === true ? 
                                        <div className="max-w-xl flex flex-col space-y-2 text-xs mx-2 order-1 items-end">
                                            <img src={mesage.pic} alt="" />
                                            <div className="text-xs text-gray-500">{cleanDate(mesage.date_created)}</div>
                                        </div>
                                    : 
                ...
                <div className="border-t-2 border-gray-200  pt-4 mb-2 sm:mb-0">
                    <div className="relative flex-col flex pb-8 overflow-scroll">
                        ...
                        <br />
                        <textarea name="userpost" onChange={(e) => setForm({...form, "posttext":e.currentTarget.value})} type="text" placeholder="Write message!" className="w-full block h-14 focus:outline-none focus:placeholder-gray-400 text-gray-600 placeholder-gray-600 pl-6 pr-6 pb-6 bg-gray-200 rounded-md py-1    scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2 scrolling-touch"></textarea>
